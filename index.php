<html><head>
    <meta charset="utf-8">
    <title>Banking System</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Petit Formal Script">
  </head>
  <body>
    <div class="header">
    
      <div class="header-logo" style="display: inline-block;margin-top:10px; font-family:Petit Formal Script;font-size:27px;color:#F8FFF8;">JussPay</div>
      
         <div class="header-list" style="display: inline-block;float: right;">
            <ul style="display: inline-block;margin-top:20px;font-size:17px;">
              <li><a href="transfer.php" class="d">Transfer</a></li>
              <li><a href="users.php" class="d">Users</a></li>
              <li><a href="history.php" class="d">History</a></li>
             </ul>
          </div>   
      
    </div>

    <div class="main">
      <h1 id="hd">Money Transfer Made easier</h1>
      <p id="p1">Send it with one click</p>
     <a href="transfer.php"><button id="button1">Get Started</button></a>
    </div>

    <div class="footer">
     Designed and Developed by JussPay
    </div>
  
</body></html>